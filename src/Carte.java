...
public class Carte extends JPanel implements Observateur, MouseListener
{
	...
	private CaseNiveaux[][] 	casesOld; 	// Copie de la carte visible pour un éventuel retour en arrière
	private Toolkit 			tk = Toolkit.getDefaultToolkit(); // Permet de gérer le curseur

	
	private int[] 		coordSelection 	= new int[4]; 	// coordonnées d'une sélection en i,j
	private boolean 	modeSelection 	= false; 		// indique si on est en train de faire une selection
	private ArrayList<CaseNiveaux> selection = new ArrayList<CaseNiveaux>(); // Contient les cases de la sélection
	
	private boolean 	aleatoire 		= true; 		// indique si le mode aléatoire de pose est activé
	...
	public Carte(int l, int h, int lc, int hc, int nbrNiv) 
	{
		...
		for (int i = 0; i < h; i++) {
			for (int j = 0; j < l; j++) {
				cases[i][j] = new CaseNiveaux(largeurCase, hauteurCase,
						Color.blue, Color.black, nbrNiveaux);
		...
		for (int i = 0; i < h; i++) {
			for (int j = 0; j < l; j++) {
				casesOld[i][j] = new CaseNiveaux(largeurCase, hauteurCase,
						Color.blue, Color.black, nbrNiveaux);
	...
	/**
	 * Remet le curseur par défaut : l'utilisateur est à nouveau libre de sélectionner un sprite ou une case
	 */
	...
	public void update(ArrayList<Image> img) 
	{
		...
		{
			this.image = img;
			Cursor monCurseur = tk.createCustomCursor(image.get(0), new Point(15, 15),
					"sprite");
	...
	public void mouseEntered(MouseEvent arg0) 
	{
		...
		// Si on est en train de faire une sélection click-deplace : on rajoute la sélection en hover
		if(modeSelection)
		{	
			for(int i = Math.min(coordSelection[0], coordArr[0]) ; i <= Math.max(coordSelection[0], coordArr[0]) ; i++)
			{
				for(int j = Math.min(coordSelection[1], coordArr[1]) ; j <= Math.max(coordSelection[1], coordArr[1]) ; j++)
				{
	...
	{		
		...
		if((coordSelection[2] >= 0) && (coordSelection[3] >= 0) && (coordSelection[2] < hauteur) && (coordSelection[3] < largeur))
		{
			if(coordSelection[0] == coordSelection[2] && coordSelection[1] == coordSelection[3])	// Clic simple
			{
				if(arg0.isControlDown())																// Avec ctrl : on ajoute/enlève la case à la sélection
				{
					toggleSelection(cases[coordSelection[0]][coordSelection[1]]);
				}else if(arg0.getButton() == MouseEvent.BUTTON2)										// Clic molette : on annule toute la sélection
				{
					clearSelection();
				}
				else																					// Sinon : ajout/suppression image
				{
					// On sauvegarde ds old avant de faire une modif
					copieCases(cases, casesOld);
					
					if(cases[coordSelection[0]][coordSelection[1]].isChosen())							// Si on est sur une case de la sélection
					{
						for(int i = 0 ; i < selection.size() ; i++)
						{
							// click gauche : on place
							if (arg0.getButton() == MouseEvent.BUTTON1 && this.getCursor() != Cursor.getDefaultCursor() && !image.isEmpty())
							{
								if(!aleatoire)
									selection.get(i).setImage(image.get(0), nivSelected);
								else
									selection.get(i).setImage(getImageAlea(), nivSelected);
							}
					...
					}else																					// Sinon on agit seulement sur la case
					{
																												// click gauche : on place
						if (arg0.getButton() == MouseEvent.BUTTON1 && this.getCursor() != Cursor.getDefaultCursor() && !image.isEmpty())
						{
							if(!aleatoire)
								cases[coordSelection[0]][coordSelection[1]].setImage(image.get(0), nivSelected);
							else
								cases[coordSelection[0]][coordSelection[1]].setImage(getImageAlea(), nivSelected);
						}
																												// click droit : on efface
						else if (arg0.getButton() == MouseEvent.BUTTON3)
						{
							cases[coordSelection[0]][coordSelection[1]].clear(nivSelected);
						}
							
					}
				}
			}else if(arg0.getButton() == MouseEvent.BUTTON1)												// Clic spécial : avec glissement souris
			{
				if(!arg0.isControlDown())
				{
					clearSelection();
				}
				for(int i = Math.min(coordSelection[0], coordSelection[2]) ; i <= Math.max(coordSelection[0], coordSelection[2]) ; i++)
				{
					for(int j = Math.min(coordSelection[1], coordSelection[3]) ; j <= Math.max(coordSelection[1], coordSelection[3]) ; j++)
					{
						if(arg0.isControlDown())														// Si ctrl : on alterne la partie sélectionnée
						{
							toggleSelection(cases[i][j]);
						}
						else																			// Sinon : on crée une nouvelle sélection
						{
	...
	/**
	 * Copie un tableau de case de même hauteur et largeur (attributs de Carte) dans un autre
	 * 
	 * @param casesACopier
	 ...
	 */
	public void copieCases(CaseNiveaux[][] casesACopier, CaseNiveaux[][] casesOuCopier)
	{
		for (int i = 0; i < hauteur; i++) 
		{
			for (int j = 0; j < largeur; j++) 
			{
				for(int k = 1 ; k <= nbrNiveaux ; k++)
				{
					...
					{
						casesOuCopier[i][j].setImage(casesACopier[i][j].getImage(k), k);
					}else
					{
	...
	/**
	 * Permet d'annuler une action : copie le tableau de sauvegarde dans le tableau visible
	 */
...
//		printCarte(cases);
	...
	/**
	 * Affiche textuellement le contenu d'une carte : affiche i si il y a une image dans la case (i,j) au niveau k
	 * 
	 * @param casesNiv
	 *            Le tableau de case de hauteur et largeur (attributs de Carte) à afficher
	 */
	 ...
	 * @return coord
	 * 			  Les coordonnées de la case : coord[0] = i et coord[1] = j
	 */
	...
	/**
	 * Sélectionne aléatoirement une image parmis celles contenues dans l'attribut image
	 * 
	 * @return img
	 *            L'image sélectionnée aléatoirement
	 */
	public Image getImageAlea()
	{
		Image img = null;

		// On détermine si on affiche une image ou non en fonction du pourcentage de remplissage
	...
	/**
	 * Ajoute une case à la sélection si elle n'y est pas ou la retire si elle y est déjà
	 * 
	 * @param caseNiv
	 *            La case à modifier
	 */
	...
	/**
	 * Modifie l'attribut hover de la case selon le fait que la case soit sélectionnée ou non
	 * 
	 * @param caseNiv
	 *            La case à modifier
	 */
